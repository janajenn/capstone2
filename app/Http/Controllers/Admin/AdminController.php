  ...
  public function approve(Request $request, $id)
{
    ...
    try {
        ...
        DB::transaction(function () use ($request, $id, $leaveRequest, $currentApprover) {
            ...
            // ========================================================
            // DEDUCTION LOGIC BASED ON LEAVE TYPE
            // ========================================================
            if ($leaveRequest->status === 'approved') {
                $leaveType = $leaveRequest->leaveType;
                $leaveTypeCode = $leaveType->code;

                \Log::info("🔍 Processing deduction for leave type: {$leaveTypeCode}");

                // CASE 1: Standard earnable leaves (SL, VL)
                if (in_array($leaveTypeCode, ['SL', 'VL'])) {
                    ...
                    if (is_array($result) && isset($result['success'])) {
                        if ($result['success']) {
                            \Log::info("✅ Leave credits deducted for {$leaveTypeCode}. With pay: {$result['days_deducted']}, Without pay: {$result['days_without_pay']}");
                        } else {
                            \Log::warning("⚠️ Leave credit deduction failed for {$leaveTypeCode}: " . ($result['message'] ?? 'unknown error'));
                        }
                    } else {
                        \Log::info("✅ Leave credits deducted for {$leaveTypeCode} (legacy response)");
                    }
                }
                // CASE 2: Leave types that deduct from VL but also have a separate balance (e.g., Force Leave)
                elseif ($leaveType->deduct_from_vl) {
                    // Deduct from VL credits (pass target 'VL')
                    $leaveCreditService = new LeaveCreditService();
                    $creditResult = $leaveCreditService->deductLeaveCredits($leaveRequest, 'VL');

                    // Deduct from the separate leave balance (annual cap)
                    $leaveBalanceService = new LeaveBalanceService();
                    $balanceResult = $leaveBalanceService->deductLeaveBalance($leaveRequest);

                    \Log::info("📊 Force Leave deduction summary:", [
                        'vl_deduction' => [
                            'success' => $creditResult['success'] ?? false,
                            'days_with_pay' => $creditResult['days_deducted'] ?? 0,
                            'days_without_pay' => $creditResult['days_without_pay'] ?? 0,
                            'new_vl_balance' => $creditResult['new_balance'] ?? null
                        ],
                        'balance_deduction' => [
                            'success' => $balanceResult['success'] ?? false,
                            'days_deducted' => $balanceResult['days_deducted'] ?? 0,
                            'new_balance' => $balanceResult['new_balance'] ?? null
                        ]
                    ]);
                }
                // CASE 3: Other non-earnable leaves (e.g., SPL, ML, PL, etc.)
                else {
                    $leaveBalanceService = new LeaveBalanceService();
                    ...
                    if (is_array($result) && isset($result['success'])) {
                        if ($result['success']) {
                            \Log::info("✅ Leave balance deducted for {$leaveTypeCode}. Days deducted: {$result['days_deducted']}");
                        } else {
                            \Log::warning("⚠️ Leave balance deduction failed for {$leaveTypeCode}: " . ($result['message'] ?? 'unknown error'));
                        }
                    } else {
                        \Log::info("✅ Leave balance deducted for {$leaveTypeCode}");
                    }
                }

                // ========================================================
                // ALWAYS REGENERATE MONTHLY RECORDINGS FOR AFFECTED YEAR(S)
                // ========================================================
                $leaveRecordingService = app(LeaveRecordingService::class);
                $yearFrom = \Carbon\Carbon::parse($leaveRequest->date_from)->year;
                $yearTo   = \Carbon\Carbon::parse($leaveRequest->date_to)->year;

                $leaveRecordingService->refreshForEmployeeYear($leaveRequest->employee_id, $yearFrom);
                if ($yearTo !== $yearFrom) {
                    $leaveRecordingService->refreshForEmployeeYear($leaveRequest->employee_id, $yearTo);
                }

                \Log::info("📅 Monthly recordings refreshed for employee {$leaveRequest->employee_id} (years: {$yearFrom}" . ($yearTo !== $yearFrom ? ", {$yearTo}" : "") . ")");
            }
